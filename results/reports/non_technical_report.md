...
- **More complex vocabulary** (higher type-token ratio): 11% increase, statistically significant (p < 0.001)
- **Longer average words** (4.3 vs 4.0 characters): 7% increase, statistically significant (p < 0.001)
- **More URLs and web links** (2.7 times more): 175% increase, statistically significant (p < 0.001)
- **Similar overall email length**: NO significant difference in word count, sentence count, or character count (p > 0.05)

**Important Correction**: **Phishing emails are NOT longer than legitimate emails**. This is a common misconception. Our statistical analysis shows negligible differences in email length (Cohen's d = -0.06, not statistically significant). The key differences are in word choice and complexity, not overall length.
...
### Statistical Confidence and Significance Testing

We have high confidence in our findings based on rigorous statistical analysis:

**Statistical Rigor**: We conducted proper statistical significance testing using two-sample t-tests, calculated 95% confidence intervals, and applied multiple comparison corrections. This goes beyond simple descriptive statistics to test whether observed differences are statistically meaningful.

**P-values and Significance Levels**:

- **Highly significant** (p < 0.001): Sentiment differences, vocabulary complexity, exclamation usage, word length, psychological manipulation terms
- **Significant** (p < 0.01): Most feature differences
- **Not significant** (p > 0.05): Email length measures (word count, sentence count, character count)

**Effect Sizes with Confidence Intervals**: We report Cohen's d effect sizes with 95% confidence intervals, providing both the magnitude of differences and our uncertainty about the true values.
